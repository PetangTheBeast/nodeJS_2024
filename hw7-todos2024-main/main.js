...
import { createWebSocketServer, sendTodoListToAllConnections, sendTodoToAllConnections } from './src/websockets.js'
...
app.post('/update-todo/:id', async (req, res, next) => {
  ...
  await query
  sendTodoListToAllConnections()
  sendTodoToAllConnections(todo.id)
  res.redirect('back')
})
...
app.get('/remove-todo/:id', async (req, res) => {
  ...
  if (!todo) return next()
  sendTodoToAllConnections(todo.id, true)
